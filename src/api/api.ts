...
import { SHA256, core } from "crypto-js";
import {
  PrismaClient,
  Prisma,
  BorrowDetails,
...
app.post("/login", bodyparsee, async (req: Request, res: Response) => {
  ...
  FetchPass(username, Hash).then((response) => {
    if (response) {
      console.log(response);
...
app.post("/updateStudent", (req: Request, res: Response) => {
  ...
  const updateUser = prisma.students
    .update({
      //By email
      where: {
...
app.post("/borrow", (req, res) => {
  ...
  const borrow = prisma.borrowDetails
    .create({
      data: borrowBook,
    })
    .then(() => {
...
app.post("/registerBook", async (req: Request, res: Response) => {
  ...
  const Book = await prisma.library
    .create({
      data: book,
    })
    .then(() => {
